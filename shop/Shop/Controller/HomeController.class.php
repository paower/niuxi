...
/* var_dump($photo);die;*/
      if(empty($data['username'])){
            echo '<script>alert("请输入充值积分账号"); window.history.back(-1); </script>';
...
public function cation()
	{
		...
		$oneCateList = M("product_cate")->where(array("pid" => 0, 'is_duobao' => 1))->field('id,pid,pic,name,type')->order('sort desc')->select();
