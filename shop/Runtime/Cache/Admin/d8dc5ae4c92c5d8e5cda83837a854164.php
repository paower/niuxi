                                ...
                                <!-- 工具栏按钮 -->
                                    <div class="col-xs-12 col-sm-8 button-list clearfix">
                                        <div class="form-group">
                                            <a title="新增" class="btn btn-primary-outline btn-pill" href="<?php echo U('Manage/add');?>">新增</a>&nbsp;
                                            <a title="启用" target-form="ids" class="btn btn-success-outline btn-pill ajax-post confirm" model="Manage" href="<?php echo U('Manage/setStatus',array('status'=>resume,'model'=>Manage));?>">启用</a>&nbsp;
                                            <a title="禁用" target-form="ids" class="btn btn-warning-outline btn-pill ajax-post confirm" model="Manage" href="<?php echo U('Manage/setStatus',array('status'=>forbid,'model'=>Manage));?>">禁用</a>&nbsp;
                                            <a title="删除" target-form="ids" class="btn btn-danger-outline btn-pill ajax-post confirm" model="Manage" href="<?php echo U('Manage/setStatus',array('status'=>delete,'model'=>Manage));?>">删除</a>&nbsp;                   
                                        </div>
                                    </div>
                                    ...
                                    <form class="form" method="get" action="">
                                        <div class="form-group">
                                            <div class="input-group search-form">
                                                <input type="text" name="keyword" class="search-input form-control" value="<?php echo ($_GET["keyword"]); ?>" placeholder="请输入ID/用户名／昵称">
                                        ...
                                        <div class="panel panel-default table-responsive">
                                            <table class="table table-bordered table-striped table-hover">
                                              <thead>
                                                ...
                                                <tbody>
                                                    <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i; if(($data['id']) != "1"): ?><tr>
                                                            <td>
                                                            ...
                                                            <td>
                                                                 <a name="edit" title="编辑" class="label label-primary-outline label-pill" href="<?php echo U('Manage/edit',array('id'=>$data['id']));?>">编辑</a>
                                                                <?php if(($data['id']) != "1"): if(($data['status']) == "1"): ?><a name="forbid" title="禁用" class="label label-warning-outline label-pill ajax-get confirm" href="<?php echo U('Manage/setStatus',array('status'=>forbid,'ids'=>$data['id'],'model'=>'Manage'));?>">禁用</a>
                                                                <?php else: ?>
                                                                <a name="forbid" title="启用" class="label label-success-outline label-pill ajax-get confirm" href="<?php echo U('Manage/setStatus',array('status'=>resume,'ids'=>$data['id'],'model'=>'Manage'));?>">启用</a><?php endif; ?>

                                                                <a name="delete" title="删除" class="label label-danger-outline label-pill ajax-get confirm" model="Manage" href="<?php echo U('Manage/setStatus',array('status'=>delete,'ids'=>$data['id'],'model'=>'Manage'));?>">删除</a><?php endif; ?>
