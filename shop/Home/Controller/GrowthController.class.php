...
//            }
            ...
            if($id_Uid != $uid){
                ajaxReturn('对不起,该张银行卡支付宝号不是您的哦~',0);
            }
            ...
            // if($is_enough > 10){
            //     // 扣除10元押金
            //     M('store')->where(array('uid'=>$uid))->setDec('cangku_num',10);
            // }else{
            //     ajaxReturn('请备好10押金',0);
            // }
...
//                $doDec = M('store')->where(array('uid'=>$uid))->setDec('cangku_num',$sellnums);
        ...
        if(IS_AJAX){
			...
			if($type == 1){
				...
				if($size){
					...
					if($res){
						ajaxReturn('提交支付宝账号成功',1,'/Growth/Cardinfos');
					}else{
						ajaxReturn('提交支付宝失败',0);
					}
				}
			}else if($type == 2){
				...
				$data['bank_name2'] = $bank_name2;
				$data['bank_type'] = 2;
				$res = M('ubanks')->add($data);
				if($res){
					ajaxReturn('提交银行卡号成功',1,'/Growth/Cardinfos');
				}else{
					ajaxReturn('提交银行卡失败',0);
				}
			}else if($type == 3){
				$picname = $_FILES['uploadfile']['name'];
				$picsize = $_FILES['uploadfile']['size'];
				...
				if($size){
					...
					$res = M('ubanks')->add($data);
					if($res){
						ajaxReturn('提交微信账号成功',1,'/Growth/Cardinfos');
					}else{
						ajaxReturn('提交微信账号失败',0);
					}
				}
			}
        }
	...
	public function edit_bank()
	{
		...
		if(IS_AJAX){
			...
			if($type == 1){
				...
				if($size){
					...
					$data['alipay_number'] = $alipay_number;
					$data['img'] = $pic_path;
					$res = M('ubanks')->where(array('id'=>$id,'user_id'=>$uid))->save($data);
					if($res){
						ajaxReturn('提交支付宝账号成功',1,'/Growth/Cardinfos');
					}else{
						ajaxReturn('提交支付宝失败',0);
		...
		$ubank = M('ubanks')->where('id='.$id)->find();
		$bakinfo = M('bank_name')->order('q_id asc')->select();
		$this->assign('bakinfo',$bakinfo);
		$this->assign('ubank',$ubank);
		$this->display();
	}
...
//卖入中心
    ...
    public function Cardinfos(){
        $uid = session('userid');
		//支付宝
        $morecars = M('ubanks as u')->where(array('u.user_id'=>$uid,'bank_type'=>1))->order('u.id desc')->field('u.alipay_name,u.id,u.alipay_number,u.user_id,u.img,u.is_default')->select();
		//银行卡
		$morecars2 = M('ubanks as u')
		->join('ysk_bank_name ON ysk_bank_name.q_id = u.card_id')
		->where(array('u.user_id'=>$uid,'bank_type'=>2))
		->order('u.id desc')
		->field('u.alipay_name,u.id,u.alipay_number,u.user_id,u.img,u.is_default,ysk_bank_name.banq_img')
		->select();
		//微信
		$morecars3 = M('ubanks as u')
		->where(array('u.user_id'=>$uid,'bank_type'=>3))
		->order('u.id desc')
		->field('u.alipay_name,u.id,u.alipay_number,u.user_id,u.img,u.is_default')
		->select();
        if(IS_AJAX){
            $cardid = I('bangid');
            //是否是自己绑定的银行卡
            ...
            if($isuid != $uid){
                ajaxReturn('该收款账号暂不属于您~',0);
            }
            $res = M('ubanks')->where(array('id'=>$cardid))->delete();
            if($res){
                ajaxReturn('该收款账号删除成功',1,'/User/Personal');
            }
        }
        $this->assign('morecars',$morecars);
		$this->assign('morecars2',$morecars2);
		$this->assign('morecars3',$morecars3);
        $this->display();
    }
    ...
    public function default_bank(){
        ...
        $ubanks = M('ubanks')->where(array('user_id'=>$userid,'id'=>$id))->setField('is_default',1);
        if($ubanks !== false){
            ajaxReturn('设置默认收款方式成功',1,'/User/Personal');
        }else{
            ajaxReturn('设置默认收款方式失败',0);
        }
    }
}