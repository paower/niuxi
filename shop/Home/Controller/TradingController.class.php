   ...
   public function SellCentr(){
        ...
        if(empty($cid)){
            ...
            if($carinfo < 1){
				...
				->where(array('u.user_id'=>$uid,'u.is_default'=>1))
				->limit(1)
				->field('u.*')
				->find();
				if(empty($morecars)){
					$morecars = M('ubanks')->where(array('user_id'=>$uid,'is_default'=>1))->find();
				}
            }
        }else{
   //          $morecars = M('ubanks as u')
			...
			// ->limit(1)
			// ->field('u.alipay_name,u.id,u.alipay_number,u.user_id,banks.banq_genre')
			// ->find();
        }
        ...
        if(IS_AJAX){
            $pwd = trim(I('pwd'));
            $sellnums = trim(I('sellnums'));//1:星座,2:推广
			$zichang = trim(I('zichang'));//金额
            $uid = session('userid');
            $is_real_name = M('user')->where('userid='.$uid)->getField('is_real_name');
            if($is_real_name==0){
                ajaxReturn('请先进行实名验证',0);
            }
            $alipay = M('ubanks')->where(array('user_id'=>$uid))->find();
            if(!alipay){
                ajaxReturn('请先绑定收款账户',0);
            }
            $zhuanrang_min = M('config')->where('name="zhuanrang_min"')->getField('value');
            $zhuanrang_max = M('config')->where('name="zhuanrang_max"')->getField('value');
            $tzhuanrang_min = M('config')->where('name="tzhuanrang_min"')->getField('value');
            $tzhuanrang_max = M('config')->where('name="tzhuanrang_max"')->getField('value');
            if($sellnums==1){
                if($zichang<$zhuanrang_min){
                    ajaxReturn('转让金额不能低于'.$zhuanrang_min,0);
                }
                if($zichang>$zhuanrang_max){
                    ajaxReturn('转让金额不能高于'.$zhuanrang_max,0);
                }
            }else{
                if($zichang<$tzhuanrang_min){
                    ajaxReturn('转让金额不能低于'.$tzhuanrang_min,0);
                }
                if($zichang>$tzhuanrang_max){
                    ajaxReturn('转让金额不能高于'.$tzhuanrang_max,0);
                }
            }
			...
			if($sellnums == 1){
				...
				if($zichang  > $is_enough){
					ajaxReturn('您当前星座收益暂无这么多余额~',0);
				}
				$record_type = 2;
			}else if($sellnums == 2){
				//自己是否有足够余额
			...
			$data['out_now_nums'] = $is_enough;
            $data['record_no'] = build_order_no();
            $res_Add = M('record')->add($data);

           
            //添加卖出余额记录
            $transdata['pay_id'] = $uid;
            $transdata['get_nums'] = $zichang;
            $transdata['get_time'] = time();
			$transdata['now_nums'] = $is_enough-$zichang;
            if($sellnums == 1){
                $transdata['get_type'] = 15;
            }else{
                $transdata['get_type'] = 16;
            }
            
            $addpid = M('tranmoney')->add($transdata);
            //给自己减少这么多余额
            if($res_Add){
                if($sellnums == 1){
        ...
        $tzhuanrang_min = M('config')->where('name="tzhuanrang_min"')->getField('value');
        $tzhuanrang_max = M('config')->where('name="tzhuanrang_max"')->getField('value');
        $this->assign('zhuanrang_min',$zhuanrang_min);
        $this->assign('zhuanrang_max',$zhuanrang_max);
        $this->assign('tzhuanrang_min',$tzhuanrang_min);
        $this->assign('tzhuanrang_max',$tzhuanrang_max);
